-- (Link for my raw data: https://github.com/AmitLahat/Portfolio-DataAnalyst/blob/main/Lotto.csv)
  ...
  BEGIN
	...
	IF @n2<10
		...
		select @n1, @n2,	
			count(Drawn_Numbers) from v_lotto_results 
			where Drawn_Numbers LIKE '%'+concat(@n1,'.')+'%'
			and Drawn_Numbers LIKE '%'+cast(@n2 as varchar(2))+'%'							
		set @n2+=1
		END
	END
	set @n1+=1
  ...
  BEGIN
	...
	IF @n2<10
		...
		select @n1, @n2,	
			count(Drawn_Numbers) from v_lotto_results 
			where (Drawn_Numbers LIKE '%'+concat('.',@n1,'.')+'%'
			Or Drawn_Numbers LIKE concat(@n1,'.')+'%')
			and Drawn_Numbers LIKE '%'+cast(@n2 as varchar(2))+'%'							
		set @n2+=1
		END
	END
...
SELECT * FROM @result
order by 3 desc
/* Can clearly see that the most common pairs are 9 and another single digit number (3,2,1,4,5)
   The results are the same for 10 past years aswell (checked in a different query file)*/
---------------------------------------------------------------------------------------------------
